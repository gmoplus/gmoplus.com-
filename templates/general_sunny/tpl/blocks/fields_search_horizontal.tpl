    ...
    <div class="search-form-cell {$cell_class} {if $field.Type != 'date'}{$field.Type}{/if}">
        <div>
            <span>
                ...
                {elseif $field.Type == 'price'}
                    ...
                    {assign var='currency_count' value=$currency_source|@count}
                    {assign var='currency_first' value=$currency_source|@current}

                    {if $lang.$field_phrase_key}{$lang.$field_phrase_key}{else}{$lang[$field.pName]}{/if}
                    {if $currency_source && $currency_count === 1}
                        ({phrase key=$currency_first.pName})
                    {/if}
                {elseif $field.Type == 'mixed'}
                    ...
                    {/if}
                    {assign var='df_count' value=$df_source|@count}
                    {assign var='df_first' value=$df_source|@current}

                    {if $lang.$field_phrase_key}{$lang.$field_phrase_key}{else}{$lang[$field.pName]}{/if}
                    {if $df_source && $df_count === 1}
                        ({phrase key=$df_first.pName})
                    {/if}
                {else}
        ...
        {elseif $field.Type == 'bool'}
            <span class="pills d-flex">
                <label title="{$lang.all}" class="flex-fill">
            ...
            {if $field.Values && $field.Values|@count < 3}
                ...
                <span class="pills d-flex">
                    ...
                    {foreach from=$field.Values item='option' key='key' name='radioF'}
                        ...
                        <label title="{$lang[$option.pName]}" class="flex-fill">

...
<article class="item{if $listing.Featured} featured{/if}{if !$pictures_available} no-image{/if} col-sm-6 col-md-4{if !$side_bar_exists} col-xl-3{/if} {rlHook name='tplListingItemClass'}">
    <div class="navigation-column">

        <ul class="nav-column{if !$price.value} stick-top{/if}">
    ...
    <div class="main-column clearfix">
        ...
        <ul class="card-info{if $config.sf_display_fields} with-names{/if}">
            <li class="before-title">
                <span class="date">{$listing.Date|date_format:$smarty.const.RL_DATE_FORMAT}</span>
                {rlHook name='listingBeforeStats'}
                <div id="fav_{$listing.ID}" class="favorite add" title="{$lang.add_to_favorites}"><span class="icon"></span><span class="link">{$lang.add_to_favorites}</span></div>
            </li>
            <li class="title">
                <a class="link-large"
            ...
            {/if}
            {if $listing.fields.description.value}
                <li class="description">{$listing.fields.description.value}</li>
            {/if}

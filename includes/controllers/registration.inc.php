...
if (!$cur_step) {
    ...
    switch ($cur_step) {
        ...
        case 'done':
            if ($config['membership_module']) {
                $registration_plan = (int) $_SESSION['registration']['plan']['ID'];

                if (isset($_SESSION['registration']['plan'])) {
                    if ($_SESSION['registration']['plan']['Price'] <= 0) {
                        $rlAccount->upgrade($account_id, $registration_plan, false, true);
                    } else {
                        $rlMembershipPlan->updatePlanData($account_id, $registration_plan);
                    }
                }
            }

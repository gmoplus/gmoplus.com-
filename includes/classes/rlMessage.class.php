...
class rlMessage
{
    ...
    /**
     * Notify admin and user about listing changed status after listing data edit
     *
     * @since 4.10.0
     *
     * @param int   $listingID   - Related listing ID
     * @param array $accountInfo - Account data
     * @param array $listingData - Listing data, if exists
     */
    public function notifyListingPendingStatus(int $listingID, array $accountInfo, array $listingData = []): void
    {
        global $config, $reefless, $rlMail;

        if (!$listingData) {
            $listingData = $GLOBALS['rlDb']->fetch('*', ['ID' => $listingID], null, 1, 'listings', 'row');
        }
        ...
        $link = $reefless->url('listing', $listingID);
        $activation_url = RL_URL_HOME
        . ADMIN
        . '/index.php?controller=listings&action=remote_activation&id='
        . $listingID
        . '&hash=' . md5($listingData['Date']);
        $activation_link = '<a href="' . $activation_url . '">' . $activation_url . '</a>';

        $listingTitle = $GLOBALS['rlListings']->getListingTitle($listingData['Category_ID'], $listingData);

...
class ManageListing extends AbstractSteps
{
    ...
    protected function simulateListingInPost($hook_name = 'addListingPostSimulation')
    {
        ...
        $options = ListingData::getOptions($this->listingID);

        // main form data simulation
        foreach ($this->formFields as &$field) {
            if ($listing[$field['Key']] == '' && !$options[$field['Key']]) {
                continue;
            }
            ...
            switch ($field['Type']) {
                ...
                case 'price':
                    ...
                    if ($config['price_separator'] != '.' && !empty($price[0])) {
                    ...
                    if ($field['Opt1'] && $options[$field['Key']]) {
                        $_POST['f'][$field['Key']] = array_merge($_POST['f'][$field['Key']], $options[$field['Key']]);
                    }
                    break;

...
var importExportClass = function(){
    ...
    this.import = function(index){
        /* import request */
        var import_url = rlConfig['plugins_url'] ? rlConfig['plugins_url'] : rlConfig['libs_url'].replace('libs/', 'plugins/');
        request = $.getJSON(import_url + 'export_import/import.php', {index: index}, function(response){
            index = response['from'];

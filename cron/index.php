...
if (empty($searches)) {
    ...
    /* prefer email notification template */
    foreach ($languages as $key => $language) {
    ...
    foreach ($searches as $key => $search) {
        ...
        if (!empty($matches)) {
            $search_account_data = $rlAccount->getProfile((int) $search['Account_ID']);
            $allow_send   = false;
            $links        = '';
            ...
            foreach ($matches as $match) {
                if (!in_array($match['ID'], $exploded_matches)) {
                    ...
                    if ($match['Account_ID'] != $search_account_data['ID']) {
                        $checked_listings .= (!empty($checked_listings) ? ',' : '') . $match['ID'];
                        $match_count++;
                        $allow_send = true;

                        $link  = $reefless->getListingUrl($match, $search_account_data['Lang']);
            ...
            if ($allow_send) {
                ...
                $count = $match_count;
                $copy_notify_email = $saved_search_email[$search_account_data['Lang'] ? $search_account_data['Lang'] : RL_LANG_CODE];
                $copy_notify_email['body'] = str_replace(
                    array('{name}', '{count}', '{links}'),
                    array($search_account_data['Full_name'], $count, $links),
                    $copy_notify_email['body']
                );
                $copy_notify_email['subject'] = str_replace(
                    array('{name}', '{count}'),
                    array($search_account_data['Full_name'], $count),
                    $copy_notify_email['subject']
                );
                $rlMail->send($copy_notify_email, $search_account_data['Mail']);

                // cron send save search
                $rlHook->load('cronSavedSearchNotify', $search, $checked_listings, $search_account_data);
            }
        }
...
// Remove expired incomplete hash listings
$sql = "SELECT * FROM `{db_prefix}listings_incomplete` WHERE DATE_ADD(`Date`, INTERVAL 14 DAY) < NOW() LIMIT 100";
if ($incomplete_hash_listings = $rlDb->getAll($sql)) {
    foreach ($incomplete_hash_listings as $hash_listing) {
        if ($mediaPath = $rlDb->getOne('Original', "`Listing_ID` = {$hash_listing['Listing_ID']} AND `Original` != 'youtube'", 'listing_photos')) {
            ListingMedia::removeEmptyDir(RL_FILES . dirname($mediaPath), true);
            $rlDb->delete(['Listing_ID' => $hash_listing['Listing_ID']], 'listing_photos', null, 0);
            $rlDb->delete(['Listing_ID' => $hash_listing['Listing_ID']], 'listings_incomplete', null, 0);
        }
    }
}

...
<div id="nav_bar">
    ...
    {if $aRights.$cKey.add}
        <a href="{$rlBaseC}action=add" class="button_bar"><span class="left"></span><span class="center-add">{$lang.add_group}</span><span class="right"></span></a>
    {/if}
...
{if $smarty.get.action}
    ...
    <form onsubmit="return submitHandler()" action="{$rlBaseC}action={if $smarty.get.action == 'add'}add{elseif $smarty.get.action == 'edit'}edit&amp;group={$smarty.get.group}{/if}" method="post">
        ...
        <tr>
            ...
            <td>
                {if $sPost.display == '1'}
                    {assign var='display_yes' value='checked="checked"'}
                {elseif $sPost.display == '0'}
                    {assign var='display_no' value='checked="checked"'}
                {else}
                    {assign var='display_yes' value='checked="checked"'}
                {/if}
                <label><input {$display_yes} class="lang_add" type="radio" name="display" value="1" /> {$lang.yes}</label>
                <label><input {$display_no} class="lang_add" type="radio" name="display" value="0" /> {$lang.no}</label>
            </td>
        </tr>
        
        <tr>
            <td class="name">{$lang.divide_into_columns}</td>
            <td>
                {if $sPost.divide_into_columns == '1'}
                    {assign var='divide_into_columns_yes' value='checked="checked"'}
                {elseif $sPost.divide_into_columns == '0'}
                    {assign var='divide_into_columns_no' value='checked="checked"'}
                {else}
                    {assign var='divide_into_columns_no' value='checked="checked"'}
                {/if}
                <label><input {$divide_into_columns_yes} class="lang_add" type="radio" name="divide_into_columns" value="1" /> {$lang.yes}</label>
                <label><input {$divide_into_columns_no} class="lang_add" type="radio" name="divide_into_columns" value="0" /> {$lang.no}</label>
            </td>
        </tr>

        <tr>
            <td class="name">{$lang.group_heading}</td>
            <td>
                {if $sPost.header == '1'}

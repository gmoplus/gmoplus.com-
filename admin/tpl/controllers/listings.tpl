...
{if !$deny}
    ...
    <script type="text/javascript" src="{$smarty.const.RL_LIBS_URL}jquery/jquery.qtip.js?rev={$config.static_files_revision}"></script>
    ...
    <script type="text/javascript" src="{$smarty.const.RL_LIBS_URL}ckeditor/ckeditor.js?rev={$config.static_files_revision}"></script>
    ...
    <div id="nav_bar">
        ...
        {if $aRights.$cKey.add && !isset($smarty.get.action)}
            <a href="javascript:void(0)" onclick="show('new_listing', '#action_blocks div');" class="button_bar"><span class="left"></span><span class="center-add">{$lang.add_listing}</span><span class="right"></span></a>
        {/if}
    ...
    {if $smarty.get.action == 'add'}
        ...
        <div style="margin: 5px 10px 10px;">
            <form onsubmit="return submitHandler()" id="add_listing" method="post" action="{$rlBase}index.php?controller={$smarty.get.controller}&amp;action=add&amp;category={$smarty.get.category}" enctype="multipart/form-data">
                ...
                <script type="text/javascript" src="{$smarty.const.RL_LIBS_URL}javascript/crossed.js?rev={$config.static_files_revision}"></script>
        ...
        <form onsubmit="return submitHandler()" id="edit_listing" method="post" action="{$rlBase}index.php?controller={$smarty.get.controller}&amp;action=edit&amp;id={$smarty.get.id}{if $smarty.get.ui}&amp;ui={$smarty.get.ui}{/if}{if $smarty.get.cat_id}&amp;cat_id={$smarty.get.cat_id}{/if}" enctype="multipart/form-data">
            ...
            <div id="crossed_area" {if !$plan_info.Cross}class="hide"{/if}>
                ...
                <fieldset class="light">
                    ...
                    <div id="crossed">
                        ...
                        var ca_post = {if $crossed}[{foreach from=$crossed item='crossed_cat' name='crossedF'}['{$crossed_cat}']{if !$smarty.foreach.crossedF.last},{/if}{/foreach}]{else}false{/if};
                        var cc_parentPoints = {if $parentPoints}[{foreach from=$parentPoints item='parent_point' name='parentF'}['{$parent_point}']{if !$smarty.foreach.parentF.last},{/if}{/foreach}]{else}false{/if};
                        var listing_owner_id = {$listing_info.Account_ID};
            ...
            {if $group.Group_ID}
                ...
                <legend id="legend_group_{$group.Key}" class="up" onclick="fieldset_action('group_{$group.Key}');">{$lang[$group.pName]}</legend>
                    <div id="group_{$group.Key}">
                    {if $group.Fields}
                        {include file='blocks'|cat:$smarty.const.RL_DS|cat:'field.tpl' fields=$group.Fields itemId=$listing_info.ID}
                    {else}
    ...
    {elseif $smarty.get.action == 'photos'}
        ...
        <style type="text/css">
        @import url("{$smarty.const.RL_LIBS_URL}cropper/cropper.css?rev={$config.static_files_revision}");
        </style>

        <script src="{$smarty.const.RL_LIBS_URL}cropper/cropper.min.js?rev={$config.static_files_revision}"></script>
        ...
        <script src="{$rlTplBase}js/crop.js?rev={$config.static_files_revision}"></script>
                    ...
                    {else}
                        ...
                        {foreach from=$videos item='video'}
                            <li id="video_{$video.ID}">
                                {if $video.video_key}
                                    <img class="preview_item" src="{$video.Thumbnail}" alt="" />
                                {else}
                                    <video controls>
                ...
                <td class="sidebar">
                    {if $photos}
                        ...
                        {foreach from=$photos item='photo' name='photosF'}
                            <li class="media-{$photo.Type}">
                                {if !$photo.video_key && $photo.Type == 'video'}
                                    <video><source src="{$photo.href}" type="video/mp4"></source></video>
                                {else}
                                    <img src="{if $smarty.foreach.photosF.first && $photo.Type == 'image'}{$photo.Photo}{else}{$photo.Thumbnail}{/if}" />
                                {/if}
                             </li>
                        {/foreach}
                        </ul>
                    ...
                    <ul class="statistics"{if !$photos} style="padding-top: 0;"{/if}>

...
<form onsubmit="return submitHandler();" action="{$rlBaseC}action={if $smarty.get.action == 'add'}add{elseif $smarty.get.action == 'edit'}edit&amp;field={$smarty.get.field}{/if}" method="post">
    ...
    <!-- price field -->
    {if $cInfo.Controller == 'listing_fields'}
        <div id="field_price" class="hide">
            <table class="form">
            <tr>
                <td class="name">{phrase key='data_formats+name+price_options'}</td>
                <td class="field">
                    {if $sPost.price.options == '1'}
                        {assign var='options_yes' value='checked="checked"'}
                    {elseif !$sPost.price.options  || $sPost.price.options == '0'}
                        {assign var='options_no' value='checked="checked"'}
                    {/if}

                    <label>
                        <input type="radio" name="price[options]" value="1" {$options_yes} /> {$lang.yes}
                    </label>
                    <label>
                        <input type="radio" name="price[options]" value="0" {$options_no} /> {$lang.no}
                    </label>

                    <span class="field_description" id="field_autocomplete_hint">
                        {assign var='replace' value='<a target="_blank" class="static" href="'|cat:$rlBase|cat:'index.php?controller=data_formats&mode=manage&format=price_options">$1</a>'}
                        {$lang.price_options_hint|regex_replace:'/\[(.*)\]/':$replace}
                    </span>
                </td>
            </tr>

            {rlHook name='apTplFieldsPrice'}
            </table>
        </div>
    {/if}
    <!-- price field end -->

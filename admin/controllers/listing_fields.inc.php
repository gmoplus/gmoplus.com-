...
if ($_GET['q'] == 'ext') {
    ...
    if ($_GET['action'] == 'update') {
        ...
        $rlDb->updateOne($updateData, 'listing_fields');
    ...
    $sql = "SELECT DISTINCT `T1`.`ID`, `T1`.`Key`, `T1`.`Type`, `T1`.`Required`, `T1`.`Add_page`, `T1`.`Details_page`, `T1`.`Map`, `T1`.`Status`, `T2`.`Value` AS `name` ";
